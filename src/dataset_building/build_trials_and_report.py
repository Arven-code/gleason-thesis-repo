...
def main():
    ...
    ap.add_argument("--in", dest="in_dir", required=True, help="Folder with *_with_speaker_training_echo_v2*.csv or equivalent")
    ...
    mot_iqr = float(
        per_file.query("caregiver_group=='MOT'")["y"].quantile(0.75) -
        per_file.query("caregiver_group=='MOT'")["y"].quantile(0.25)
    ) if (per_file["caregiver_group"] == "MOT").any() else float("nan")
    fat_med = float(per_file.query("caregiver_group=='FAT'")["y"].median()) if (per_file["caregiver_group"] == "FAT").any() else float("nan")
    fat_iqr = float(
        per_file.query("caregiver_group=='FAT'")["y"].quantile(0.75) -
        per_file.query("caregiver_group=='FAT'")["y"].quantile(0.25)
    ) if (per_file["caregiver_group"] == "FAT").any() else float("nan")
    ...
    parts = [
        f"Across {n_files} files, child success following MOT turns was {pct(mot_rate)} ({mot_s}/{mot_N}) versus {pct(fat_rate)} ({fat_s}/{fat_N}) after FAT turns; difference = {delta_pp:.1f} pp (Fisher’s exact: OR={OR:.2f}, p={p:.3g})."
    ]
    if not (math.isnan(mot_med) or math.isnan(fat_med)):
        parts.append(
            f"At the file level, the median success rate was {pct(mot_med)} (IQR {pct(mot_iqr)}) for MOT and {pct(fat_med)} (IQR {pct(fat_iqr)}) for FAT."
        )
    if has_ids:
        parts.append(
            f"IDS-like prosody occurred more often in {most_ids} ({pct(ids_rate_mot)} MOT vs {pct(ids_rate_fat)} FAT). Within IDS-flagged spans, success was {pct(succ_ids)} versus {pct(succ_nonids)} in non-IDS spans."
        )
    ...
    pd.DataFrame({
        ...
        "success": [mot_s, fat_s],
        "N": [mot_N, fat_N],
        "rate": [mot_rate, fat_rate]
    }).to_csv(os.path.join(out_dir, "gender_ids_group_table.csv"), index=False)
    ...
    with open(os.path.join(out_dir, "gender_ids_summary.json"), "w", encoding="utf-8") as f:
        json.dump({
            ...
            "n_files": n_files,
            "per_file": {
                "MOT_median_rate": mot_med,
                "MOT_IQR": mot_iqr,
                "FAT_median_rate": fat_med,
                "FAT_IQR": fat_iqr
            },
            "ids": {
                "available": has_ids,
                "MOT_rate": ids_rate_mot,
                "FAT_rate": ids_rate_fat,
                "succ_in_IDS": succ_ids,
                "succ_in_nonIDS": succ_nonids,
                "most_IDS_group": most_ids
            },
            "diag": diag
        }, f, indent=2)
