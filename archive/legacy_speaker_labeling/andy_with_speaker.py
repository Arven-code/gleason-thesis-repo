...
df = pd.read_csv("/mnt/data/andy_dataset.csv")
...
cha_utts = []
with open("/mnt/data/andy.cha", "r", encoding="utf-8", errors="ignore") as f:
    for line in f:
        if line.startswith("*"):
            spk, rest = line[1:].split(":", 1)
            utt = re.sub(r"\x15\d+_\d+\x15", "", rest).strip()
            cha_utts.append((spk.strip().upper(), utt))
...
out_path = "/mnt/data/andy_with_speaker.csv"
